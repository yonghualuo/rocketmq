...
public class MappedFile extends ReferenceResource {
    ...
    /**
     * 堆内存池，如果开启transientStorePoolEnable为True时启用。
     */
    ...
    /**
     * 初始化，把文件转为filechannel和mappedByteBuffer等。
     * @param fileName
     * @param fileSize
     * @throws IOException
     */
     ...
     * 内存映射文件的提交动作
     *
     * @param commitLeastPages 本次提交最小页数，如果待提交数据不满足commitLeastPages，则不执行本次提交操作，待下次提交。
     * @return
     */
    ...
    /**
     * 提交的具体实现，将MappedFile#writeBuffer中的数据提交到文件通道FileChannel中。
     * TODO ByteBuffer#slice()方法创建一个共享缓存区,与原先的ByteBuffer共享内存但维护一套独立的指针(position、mark、limit)
     *
    ...
    protected void commit0(final int commitLeastPages) {
        ...
        if (writePos - this.committedPosition.get() > 0) {
            try {
                ...
                // 把committedPosition到wrotePosition的数据写入到FileChannel中。
    ...
    protected boolean isAbleToCommit(final int commitLeastPages) {
        ...
        /**
         * 比较wrotePosition（当前writeBuffer的写指针）与上一次提交的指针（committedPosition）的差值，除以OS_PAGE_SIZ，
         * 得到当前脏页的数量
         */
    ...
    /**
     * 查找pos到当前最大可读之间的数据
     * 由于在整个写入期间都未曾改变MappedByteBuffer的指针，所以mappedByteBuffer.slice为整个MappedFile。
     *
     * @param pos
     * @param size
     * @return
    ...
    public SelectMappedBufferResult selectMappedBuffer(int pos, int size) {
        ...
        if ((pos + size) <= readPosition) {
            ...
            if (this.hold()) {
                // 调用slice方法后，再对其ByteBuffer进行读取时，一般手动指定position与limit指针，而不是调用flip方法来切换读写状态。
    ...
    /**
     * 获取最大读指针。
     * 内存映射文件，预先申请一块连续的固定大小的内存，需要一套指针标识当前最大有效数据的位置。
