...
/**
 * MappedFile的管理容器，是对存储目录的封装，该目录下会存在多个内存映射文件（MappedFile）。
 */
public class MappedFileQueue {
     ...
     * RocketMQ采取定时删除存储文件的策略，也就是说在存储文件中，第一个文件不一定是0000000000000000，因为该文件在某一时刻会被删除，故offset%mappedFileSize不可行。
